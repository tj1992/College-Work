...
if __name__ == "__main__":
	...
	# get arguments
	args = getopt.getopt(sys.argv[1:], 'k:f:')
	...
	for i in args[0]:
		if i[0] == '-k':
			...
			print ('Usage: [-k <k-parameter> -f <filename> ]')
			sys.exit(1)
	...
	centroid = Point()
	for i in range(0, k):
		centroid.x.append(points.x[i])
		centroid.y.append(points.y[i])
	
	print ('Before:')
	for i in range(0, k):
		print ('Centroid', i+1, ':', centroid.x[i], centroid.y[i])

	# calculate the centroid (max nine iterations)
	for j in range(0, 10):
		nc = Point(k)
	
		for i in range(0, len(points.x)):	# iterate over the list of points
			points.flag[i] = min_point(centroid, points.x[i], points.y[i])	# get the closest centroid point
			nc.x[points.flag[i]] += points.x[i]	# accumulate the x-coordinate value
			nc.y[points.flag[i]] += points.y[i]	# accumulate the y-coordinate value
			nc.flag[points.flag[i]] += 1	# increment the element count
